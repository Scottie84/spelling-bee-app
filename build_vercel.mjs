...
const POLY_PLACEHOLDER = '__POLY_PROBLEMS__';
...
let polyProblems = 'null';
try { polyProblems = readFileSync('poly_problems.json', 'utf8').trim(); }
catch { console.warn('[build_vercel] poly_problems.json not found — Poly mode will be empty.'); }
...
if (!template.includes(POLY_PLACEHOLDER)) {
  throw new Error(`expected ${POLY_PLACEHOLDER} placeholder in index.template.html`);
}
...
let html = template
  ...
  .replace(POLY_PLACEHOLDER, () => polyProblems); // Poly problem set (valid JSON)
